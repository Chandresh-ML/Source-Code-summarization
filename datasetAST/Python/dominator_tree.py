...
def draw_graph(node_dict, name='GRAPH', display_code=False):
    ...
    for idx, node in node_dict.items():
        # Node display source code, or display id
        if display_code:
...
def split_dom_tree(block_list, node_dict, current_node='1', current_block=None):
    ...
    if len(node_dict) == 1:  # end node
        ...
        return
    # If the current node is a branch node, then the current node is added to the current block, and the current block is added to the block_list
...
def split_code(cfg, block_list):
    ...
    for block in block_list:
        ...
        for node_id in block:
            ...
            # delte catch、finally，to generate ast
...
def get_code_blocks(cfg_path):
    ...
    # geneerate dominator tree，The predecessor of each node is immediate dominator
...
if __name__ == '__main__':
    cfg_dir = 'C:\Users\DELL\mtechproject material\TEST Models\6 run\datasetAST\Python\valid.jsonl'
    code_split_dir = 'C:\Users\DELL\mtechproject material\TEST Models\6 run\datasetAST\Python\test.jsonl'
