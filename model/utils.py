...
class Datagen_tree:
    def __init__(self, Ast1, Ast2, label, batch_size, ast_dic, train=True, binary=False):
        self.Ast1 = Ast1
        self.Ast2 = Ast2
        self.label = label
        self.batch_size = batch_size
        self.ast_dic = ast_dic
        self.train = train
        self.binary = binary

    def __len__(self):
        return len(range(0, len(self.label), self.batch_size))
    ...
    def gen(self, epoch):
        if self.train:
            ...
            self.Ast1 = read_pickle(self.Ast1)
            self.Ast2 = read_pickle(self.Ast2)
            newindex = list(np.random.permutation(len(self.Ast1)))
            Ast1 = [self.Ast1[i] for i in newindex]
            Ast2 = [self.Ast2[i] for i in newindex]
            label = [self.label[i] for i in newindex]
        else:
            self.Ast1 = read_pickle(self.Ast1)
            self.Ast2 = read_pickle(self.Ast2)
            Ast1 = [ast for ast in self.Ast1]
            Ast2 = [ast for ast in self.Ast2]
            label = [l for l in self.label]
        for i in range(0, len(label), self.batch_size):
            ast_1 = Ast1[i:i + self.batch_size]
            ast_2 = Ast2[i:i + self.batch_size]
            label_batch = label[i:i + self.batch_size]
            # print(label_batch)
            ast1 = [consult_tree(n, self.ast_dic) for n in ast_1]
            ast2 = [consult_tree(n, self.ast_dic) for n in ast_2]
            # label_batch = tf.convert_to_tensor(label_batch)
            label_batch = tf.expand_dims(label_batch, 1)

            yield tree2tensor(ast1), tree2tensor(ast2), label_batch
...
def calc_bleu(ref, translation):
    ...
    try:
        ...
        # os.remove(translation)   # 移除文件

    except: pass
    os.remove("temp")
    return bleu_score_report, score